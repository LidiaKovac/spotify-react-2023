...
import { getSongsByQuery } from "../../app/actions/home/home"
    ...
    return (
        <>
            <Col className="main" sm={12} md={9} lg={8}>
                {loading ? (
                    <Spinner className="position-absolute top-50 start-50 text-white" animation="border"/>
                ) : (
                    <>
                        <nav></nav>
                        <Jumbotron />
                        <div className="recent__container p-3">
                            ...
                            <Row className="recent w-100 g-0"></Row>
                        </div>
                        <div className="favorites p-3">
                            <h3>
                                Altro da
                                <span className="favorites__container text-capitalize">
                                    {" "}
                                    {more.length > 0 && more[0].artist.name}
                                </span>
                            </h3>
                            <Row className="favorites__results gy-2 justify-content-center">
                                {more.map((canzone) => (
                                    <Card key={canzone.id} song={canzone} />
                                ))}
                            </Row>
                        </div>
                        <div className="shows p-3">
                            <h3>
                                I tuoi preferiti da
                                <span className="shows__container text-capitalize">
                                    {" "}
                                    {fav.length > 0 && fav[0].artist.name}
                                </span>
                            </h3>
                        ...
                        <div className="liked p-3">
                            <h3>
                                Perche' ti piace
                                <span className="liked__container text-capitalize">
                                    {" "}
                                    {related.length > 0 && related[0].artist.name}
                                </span>
                            </h3>
